...
public class ContactsManager {
  ...
  private static String MIMETYPE = "vnd.android.cursor.item/com.nanda.contactsyncfromapp";
  ...
  public static void addContact(Context context, MyContact contact) {
    ...
    if (mHasAccount) {
      ...
      ops.add(ContentProviderOperation.newInsert(
          ...
          .withValue(ContactsContract.RawContacts.ACCOUNT_NAME, AppConstants.ACCOUNT_NAME)
      ...
      ops.add(ContentProviderOperation.newInsert(
          ...
          .withValue(ContactsContract.Data.DATA3, "Contacts Account Manager")
          .build());
  ...
  private static boolean isAlreadyRegistered(ContentResolver resolver, String id) {
    ...
    for (int i = 0; i < str.size(); i++) {
      Cursor c1 = resolver.query(RawContacts.CONTENT_URI, new String[] { RawContacts.ACCOUNT_TYPE },
          RawContacts._ID + "=?", new String[] { str.get(i) }, null);

      if (c1 != null) {
        c1.moveToFirst();
        String accType = c1.getString(c1.getColumnIndexOrThrow(RawContacts.ACCOUNT_TYPE));
        if (accType != null && accType.equals("com.nanda.contactsyncfromapp")) {
          isRegistered = true;
          break;
        }
        c1.close();
  ...
  public static void updateMyContact(Context context, String id) {
    ...
    Cursor ids = context.getContentResolver()
        .query(ContactsContract.Contacts.CONTENT_URI, null, ContactsContract.Contacts._ID + " = ?",
            new String[] { id }, null);
